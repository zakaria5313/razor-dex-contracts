module venas::router {
    ...
    public entry fun create_pair<X, Y>(
        ...
        if (swap_utils::sort_token_type<X, Y>()) {
            factory::create_pair<X, Y>(sender);
        } else {
            factory::create_pair<Y, X>(sender);
        }
    }
    ...
    public entry fun add_liquidity<X, Y>(
        ...
        if (!(factory::is_pair_created<X, Y>() || factory::is_pair_created<Y, X>())) {
        ...
        let _lp_amount;
        if (swap_utils::sort_token_type<X, Y>()) {
            (amount_x, amount_y, _lp_amount) = factory::add_liquidity<X, Y>(sender, amount_x_desired, amount_y_desired);
            assert!(amount_x >= amount_x_min, E_INSUFFICIENT_X_AMOUNT);
            assert!(amount_y >= amount_y_min, E_INSUFFICIENT_Y_AMOUNT);
        } else {
            (amount_y, amount_x, _lp_amount) = factory::add_liquidity<Y, X>(sender, amount_y_desired, amount_x_desired);
            assert!(amount_x >= amount_x_min, E_INSUFFICIENT_X_AMOUNT);
            assert!(amount_y >= amount_y_min, E_INSUFFICIENT_Y_AMOUNT);
        };
    }
    ...
    fun is_pair_created_internal<X, Y>(){
        assert!(factory::is_pair_created<X, Y>() || factory::is_pair_created<Y, X>(), E_PAIR_NOT_CREATED);
    }
    ...
    public entry fun remove_liquidity<X, Y>(
        ...
        let amount_y;
        if (swap_utils::sort_token_type<X, Y>()) {
            (amount_x, amount_y) = factory::remove_liquidity<X, Y>(sender, liquidity);
            assert!(amount_x >= amount_x_min, E_INSUFFICIENT_X_AMOUNT);
            assert!(amount_y >= amount_y_min, E_INSUFFICIENT_Y_AMOUNT);
        } else {
            (amount_y, amount_x) = factory::remove_liquidity<Y, X>(sender, liquidity);
            assert!(amount_x >= amount_x_min, E_INSUFFICIENT_X_AMOUNT);
            assert!(amount_y >= amount_y_min, E_INSUFFICIENT_Y_AMOUNT);
        }
    }
    ...
    fun add_swap_event_with_address_internal<X, Y>(
        ...
        if (swap_utils::sort_token_type<X, Y>()){
            factory::add_swap_event_with_address<X, Y>(sender_addr, amount_x_in, amount_y_in, amount_x_out, amount_y_out);
        } else {
            factory::add_swap_event_with_address<Y, X>(sender_addr, amount_y_in, amount_x_in, amount_y_out, amount_x_out);
        }
    }
    ...
    public entry fun swap_exact_input<X, Y>(
        ...
        let y_out = if (swap_utils::sort_token_type<X, Y>()) {
            factory::swap_exact_x_to_y<X, Y>(sender, x_in, signer::address_of(sender))
        } else {
            factory::swap_exact_y_to_x<Y, X>(sender, x_in, signer::address_of(sender))
        };
    ...
    public entry fun swap_exact_output<X, Y>(
        ...
        let x_in = if (swap_utils::sort_token_type<X, Y>()) {
            let (rin, rout, _) = factory::token_reserves<X, Y>();
            let amount_in = swap_utils::get_amount_in(y_out, rin, rout);
            factory::swap_x_to_exact_y<X, Y>(sender, amount_in, y_out, signer::address_of(sender))
        } else {
            let (rout, rin, _) = factory::token_reserves<Y, X>();
            let amount_in = swap_utils::get_amount_in(y_out, rin, rout);
            factory::swap_y_to_exact_x<Y, X>(sender, amount_in, y_out, signer::address_of(sender))
        };
    ...
    fun get_intermediate_output<X, Y>(is_x_to_y: bool, x_in: coin::Coin<X>): coin::Coin<Y> {
        if (is_x_to_y) {
            let (x_out, y_out) = factory::swap_exact_x_to_y_direct<X, Y>(x_in);
            coin::destroy_zero(x_out);
            y_out
        }
        else {
            let (y_out, x_out) = factory::swap_exact_y_to_x_direct<Y, X>(x_in);
            coin::destroy_zero(x_out);
            y_out
        }
    }
    ...
    fun get_intermediate_output_x_to_exact_y<X, Y>(is_x_to_y: bool, x_in: coin::Coin<X>, amount_out: u64): coin::Coin<Y> {
        if (is_x_to_y) {
            let (x_out, y_out) = factory::swap_x_to_exact_y_direct<X, Y>(x_in, amount_out);
            coin::destroy_zero(x_out);
            y_out
        }
        else {
            let (y_out, x_out) = factory::swap_y_to_exact_x_direct<Y, X>(x_in, amount_out);
            coin::destroy_zero(x_out);
            y_out
        }
    }
    ...
    fun swap_exact_input_double_internal<X, Y, Z>(
        ...
        let sender_addr = signer::address_of(sender);
        factory::check_or_register_coin_store<Z>(sender);
    ...
    fun swap_exact_output_double_internal<X, Y, Z>(
        ...
        let y_out = if (second_is_y_to_z) {
            (rin, rout, _) = factory::token_reserves<Y, Z>();
            swap_utils::get_amount_in(z_out, rin, rout)
        }else {
            (rout, rin, _) = factory::token_reserves<Z, Y>();
            swap_utils::get_amount_in(z_out, rin, rout)
        };
        let x_in = if (first_is_x_to_y) {
            (rin, rout, _) = factory::token_reserves<X, Y>();
            swap_utils::get_amount_in(y_out, rin, rout)
        }else {
            (rout, rin, _) = factory::token_reserves<Y, X>();
            swap_utils::get_amount_in(y_out, rin, rout)
        };
    ...
    fun swap_exact_input_triple_internal<X, Y, Z, A>(
        ...
        let sender_addr = signer::address_of(sender);
        factory::check_or_register_coin_store<A>(sender);
    ...
    fun swap_exact_output_triple_internal<X, Y, Z, A>(
        ...
        let z_out = if (third_is_z_to_a) {
            (rin, rout, _) = factory::token_reserves<Z, A>();
            swap_utils::get_amount_in(a_out, rin, rout)
        }else {
            (rout, rin, _) = factory::token_reserves<A, Z>();
            swap_utils::get_amount_in(a_out, rin, rout)
        };
    ...
    fun swap_exact_input_quadruple_internal<X, Y, Z, A, B>(
        ...
        let sender_addr = signer::address_of(sender);
        factory::check_or_register_coin_store<B>(sender);
    ...
    fun swap_exact_output_quadruple_internal<X, Y, Z, A, B>(
        ...
        let a_out = if (fourth_is_a_to_b) {
            (rin, rout, _) = factory::token_reserves<A, B>();
            swap_utils::get_amount_in(b_out, rin, rout)
        }else {
            (rout, rin, _) = factory::token_reserves<B, A>();
            swap_utils::get_amount_in(b_out, rin, rout)
        };
    ...
    public entry fun register_lp<X, Y>(sender: &signer) {
        factory::register_lp<X, Y>(sender);
    }
